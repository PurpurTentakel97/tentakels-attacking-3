...
void HLanguageManager::InitializeAvailableLanguages() {
    ...
    if (not DirectoryExists(directory.c_str())) {
        Print(PrintType::ERROR, "directory \"{}\" not found -> no language available", directory);
        return;
    }
    for (auto const& entry : std::filesystem::directory_iterator(directory)) {
        std::filesystem::path file{ entry.path().filename() };
        if (not file.has_extension()) {
            Print(PrintType::ERROR, "language file \"{}\" has no extension", file.string());
            continue;
        }
        if (file.extension() != ".tal") {
            Print(PrintType::ERROR, "file \"{}\" has wrong datatype", file.string());
            continue;
        }
        auto const& fileName{ file.replace_extension() };
        if (contains(fileName.string())) {
            Print(PrintType::ERROR, "language \"{}\" loaded multiple times", fileName.string());
            continue;
        }
...
void HLanguageManager::ChanceLanguage(std::string const& language) {
    ...
    if (not validLoad) {
        ...
        if (language != m_default_language) {
            ...
            if (validDefaultLoad) {
                Print(PrintType::ERROR, "fallback to default language: \"{}\"", m_default_language);
                handleUpdateLanguage();
                return;
            }
...
bool HLanguageManager::LoadLanguage(std::string const& language, bool const defaultLanguage) {
    ...
    if (not found) {
        ...
        Print(PrintType::ERROR, "language \"{}\" is not available", language);
        return false;
    }
    ...
    if (not DirectoryExists(directory.c_str())) {
        ...
        Print(PrintType::ERROR, "directory \"{}\" not existing. unable to load provided language", directory);
        return false;
    }
    ...
    if (not in.is_open()) {
        ...
        Print(PrintType::ERROR, "not able tp open language: \"{}\"", language);
        return false;
    }
    ...
    if (defaultLanguage) {
        Print(PrintType::INITIALIZE, "default language loaded -> \"{}\"", language);
    } else {
        AppContext::GetInstance().constants.global.currentLanguageName = language;

        Print(PrintType::INITIALIZE, "current language loaded -> \"{}\"", language);
    }
    ...
    if (dummy == nullptr) {
        assert(dummy);
        Print(PrintType::ERROR, "not able to check language version -> loaded language is nullptr -> \"{}\"", language);
        return false;
    } else if (dummy.is_null()) {
        assert(not dummy.is_null());
        Print(PrintType::ERROR, "not able to check language version -> loaded language json in null -> \"{}\"", language
        );
        return false;
...
std::pair<bool, std::string> HLanguageManager::RawText(std::string const& key, bool const defaultLanguage) const {
    ...
    if (dummy == nullptr) {
        Print(PrintType::ERROR, "No {} language loaded", dummyText);
        return { false, m_missing_language_text };
    } else if (dummy.is_null()) {
        Print(PrintType::ERROR, "{} language is null", dummyText);
        return { false, m_missing_language_text };
    } else if (not dummy.contains(key)) {
        Print(PrintType::ERROR, "{} language does not contain \"{}\"", dummyText, key);
