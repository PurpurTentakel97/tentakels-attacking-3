...
void AssetManager::LoadTitle() {
    ...
    if (!std::filesystem::exists(filename)) {
        Print(PrintType::ERROR, "title does not exists -> {}", filename);
        return;
    }
    ...
    if (!newFile.is_open()) {
        Print(PrintType::ERROR, "can't open title -> {}", filename);
        return;
    }
...
void AssetManager::LoadFont() {
    ...
    if (!std::filesystem::exists(filename)) {
        Print(PrintType::ERROR, "font does not exist -> {}", filename);
        return;
    }
...
void AssetManager::LoadFiles() {
    for (size_t i = 0; i < m_files.size(); ++i) {
        ...
        if (!std::filesystem::exists(filename)) {
            Print(PrintType::ERROR, "Assets does not exist -> {}", filename);
            continue;
        }
