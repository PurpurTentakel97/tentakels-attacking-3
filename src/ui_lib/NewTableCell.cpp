...
NewTableCell::NewTableCell(unsigned int const ID, Vector2 const pos, Vector2 const size, index_callback_ty callback)
	: Focusable{ ID }, UIElement{ pos, size, Alignment::DEFAULT }, m_indexCallback{ callback } {
	set_text_offset();
}
	...
	if (isA<std::monostate>()) { /*nothing to do here*/ }
		...
		DrawTextEx(
			*AppContext::GetInstance().assetManager.GetFont(),
			m_strValue.c_str(),
			{
				m_collider.x + m_textRenderOffset.x,
				m_collider.y + m_textRenderOffset.y
			},
			size,
			0.0f,
			WHITE
