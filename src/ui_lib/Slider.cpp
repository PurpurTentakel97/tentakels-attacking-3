...
namespace uil {
    ...
    void Slider::CalculateOnSlide() const {
        ...
        float const total{ m_isHorizontal ? m_collider.width - btnCollider.width
    ...
    void Slider::SlideIfScroll() {
        ...
        if (m_isHorizontal != (IsKeyDown(KEY_LEFT_SHIFT) || IsKeyDown(KEY_RIGHT_SHIFT))) {
            return;
        }
    ...
    void Slider::SetOffset(Vector2 const mousePosition) {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && !CheckCollisionPointRec(mousePosition, btnCollider)) {
            m_btnOffset = 0.0f;
            return;
        }
