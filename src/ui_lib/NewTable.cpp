...
bool NewTable::validSpecialIndex(size_t row, size_t column) const {
	return row <= m_row_count
		and column <= m_column_count
		and (validSpecialRow(row)
			or validSpecialColumn(column));
}
...
void NewTable::update_cell_focus_ids() {
	auto const id{ [this](size_t row, size_t column) -> size_t {
		...
		for (size_t column = first_column; column <= m_column_count; ++column) {
			if (row == first_row and column == first_column) {
				if (row == 0 and column == 0) {
					...
					m_cells[row][column]->SetPositionUnaligned(Vector2{
						m_pos.x - scroll_offset.x,
				...
				if (row == 0) {
					current->SetPositionUnaligned(
						Vector2{
							previous->GetPosition().x + previous->GetSize().x - scroll_offset.x,
				...
				if (column == 0) {
					current->SetPositionUnaligned(
						Vector2{
							...
							previous->GetPosition().y + previous->GetSize().y - scroll_offset.y
						}
					);
				}
				else {
					current->SetPositionUnaligned(
						Vector2{
							previous->GetPosition().x - scroll_offset.x,
							previous->GetPosition().y + previous->GetSize().y - scroll_offset.y
						}
					);
				}
				...
				current->SetPositionUnaligned(
					Vector2{
						top_cell->GetPosition().x,
						left_cell->GetPosition().y
					}
				);
			}
		}
...
void NewTable::clamp_scroll_offset(Vector2& offset) const {
	...
	if (m_row_count < first_row + 1) { Print(PrintType::EXPECTED_ERROR, "not enough rows in table for clamping"); return; }
	if (m_column_count < first_column + 1) { Print(PrintType::EXPECTED_ERROR, "not enough columns in table for clamping"); return; }
...
void NewTable::check_and_update_scroll(Vector2 const& mousePosition) {
	...
	Print(PrintType::DEBUG, "current offset: x: {}, y: {} | new offset: x: {}, y:{}",
		m_scroll_offset.x,
		m_scroll_offset.y,
		offset.x,
		offset.y
...
NewTable::NewTable(unsigned int ID, Vector2 pos, Vector2 size, Alignment alignment, size_t row_count, size_t column_count, float text_size)
	: Focusable{ ID }, UIElement{ pos, size, alignment }, m_row_count{ row_count }, m_column_count{ column_count }, m_preference_text_size{ text_size } {
...
bool NewTable::removeColumn(size_t column) {
	if (m_column_count == 0) { return false; }
