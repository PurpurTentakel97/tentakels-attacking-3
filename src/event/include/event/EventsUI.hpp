...
public:
    SetTargetFPSEvent(size_t fps) : m_fps{ fps } { }
...
public:
    ToggleFullscreenEvent(bool const isNextFullscreen) : m_isNextFullscreen{ isNextFullscreen } { }
...
public:
    SetNewResolutionEvent(Resolution resolution) : m_resolution{ resolution } { }
...
public:
    PlaySoundEvent(SoundType soundType) : m_soundType{ soundType } { }
...
public:
    SetMasterVolumeEvent(float level) : m_level{ level } {};
...
public:
    MuteMasterVolumeEvent(bool mute) : m_mute{ mute } { }
...
public:
    FocusEvent(Focusable_ty_raw focusable) : m_focusable{ focusable } { }
...
public:
    RenderFocusEvent(bool render) : m_render{ render } { }
...
public:
    SwitchSceneEvent(SceneType sceneType) : m_sceneType{ sceneType } { }
...
public:
    ClosePopUpEvent(PopUp* popUp) : m_popUp{ popUp } { }
...
public:
    PopUpEvent(std::string const& title, std::string const& subTitle) : m_title{ title }, m_subTitle{ subTitle } { }
...
public:
    ShowMessagePopUpEvent(std::string const& title, std::string const& subTitle, std::function<void()> callback)
        : PopUpEvent{ title, subTitle },
          m_callback{ callback } { }
...
public:
    ShowFightResultEvent(HFightResult result, callback_ty callback) : m_result{ result }, m_callback{ callback } { }
...
public:
    ChangeLanguageEvent(std::string const& language) : m_language{ language } { }
...
public:
    UpdateLanguageInUIEvent(std::string const& language) : m_language{ language } { }
...
public:
    DragLineFleetInstructionEvent(unsigned int originID, unsigned int destID, vec2pos_ty destCoordinates)
        : m_originID{ originID },
          m_destID{ destID },
          m_destCorrinates{ destCoordinates } { }
