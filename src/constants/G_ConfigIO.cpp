...
namespace cst {
    void G_ConfigIO::LoadConfig() {
        ...
        if (nlohmann::json son; LoadSection(load, son, G_ConfigEnum::G_FLEET, G_Fleet::s_total_config_entry_count)) {
            if (utl::usize out; LoadUSize(son, out, G_ConfigEnum::G_FLEET_MIN_FLEET_SPEED)) { constants.g_fleet.m_min_fleet_speed = out; }
            if (utl::usize out; LoadUSize(son, out, G_ConfigEnum::G_FLEET_CURRENT_FLEET_SPEED)) { constants.g_fleet.m_current_fleet_speed = out; }
            if (utl::usize out; LoadUSize(son, out, G_ConfigEnum::G_FLEET_MAX_FLEET_SPEED)) { constants.g_fleet.m_max_fleet_speed = out; }
        }
    ...
    void G_ConfigIO::SaveConfig() {
        ...
        save[CToS(G_ConfigEnum::G_FLEET)] = {
            { CToS(G_ConfigEnum::G_FLEET_TOTAL_CONFIG_ENTRY_COUNT), constants.g_fleet.s_total_config_entry_count },
            { CToS(G_ConfigEnum::G_FLEET_LOAD_CONFIG_ENTRY_COUNT), constants.g_fleet.s_load_config_entry_count },
            { CToS(G_ConfigEnum::G_FLEET_MIN_FLEET_SPEED), constants.g_fleet.m_min_fleet_speed },
            { CToS(G_ConfigEnum::G_FLEET_CURRENT_FLEET_SPEED), constants.g_fleet.m_current_fleet_speed },
            { CToS(G_ConfigEnum::G_FLEET_MAX_FLEET_SPEED), constants.g_fleet.m_max_fleet_speed },
        };
