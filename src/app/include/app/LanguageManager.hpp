...
namespace app {
    class LanguageManager final : public eve::EventListener {
    private:
    ...
    template<typename... Args>
    inline std::string LanguageManager::ReplacePlaceholders(std::string_view const text, Args const&... args) const {
        try {
            ...
            hlp::Print(hlp::PrintType::ERROR, R"(bad alloc while language text -> "{}")", text);
    ...
    template<typename... Args>
    inline std::string LanguageManager::Text(std::string const& key, Args const&... args) const {
