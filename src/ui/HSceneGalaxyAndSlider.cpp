...
namespace ui {
    void GalaxyScene::Initialize(bool const isShowGalaxy, bool const isAcceptingInput) {
        ...
        m_galaxy = std::make_shared<UIGalaxy>(1000,
                                              ...
                                              isAcceptingInput);
        m_galaxy->SetOnZoom(
                [this](float scaleFactor, Vector2 referenceScale) { this->Zoom(scaleFactor, referenceScale); });
        m_galaxy->SetOnSlide([this](float position, bool isHorizontal) { this->Slide(position, isHorizontal); });
        m_elements.push_back(m_galaxy);

        // slider
        m_verticalSlider = std::make_shared<uil::Slider>(GetElementPosition(0.00f, 0.46f),
        ...
        m_horizontalSlider = std::make_shared<uil::Slider>(GetElementPosition(0.5175f, 1.0f),
        ...
        // btn
        m_zoomInBtn = std::make_shared<uil::ClassicButton>(1001,
        ...
        m_zoomOutBtn = std::make_shared<uil::ClassicButton>(1002,
        ...
        m_scaleLineX = std::make_shared<uil::Line>(
                GetElementPosition(0.041f, 0.94f), GetElementPosition(0.241f, 0.94f), 2.0f, WHITE);

        m_scaleLineY = std::make_shared<uil::Line>(
                GetElementPosition(0.036f, 0.73f), GetElementPosition(0.036f, 0.93f), 2.0f, WHITE);
    ...
    GalaxyScene::GalaxyScene(Vector2 const pos,
                             Vector2 const size,
