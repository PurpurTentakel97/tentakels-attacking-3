...
namespace ui {
    void GameEventSettings::Initialize(utl::usize const focusID) {
        ...
        // Title
        auto title = std::make_shared<uil::Text>(GetElementPosition(0.5f, 0.0f),
        ...
        m_elements.push_back(std::make_shared<uil::Line>(
                GetElementPosition(0.3f, 0.15f), GetElementPosition(0.7f, 0.15f), 3.0f, WHITE));
        ...
        for (utl::usize i = 0; i < m_text.size(); ++i) {
            auto displayText =
                    std::make_shared<uil::Text>(GetElementPosition(textX, firstRow + row * static_cast<float>(i) * 2),
            ...
            auto element =
                    std::make_shared<uil::CheckBox>(i + focusID,
            ...
            element->SetChecked(appContext.constants.h_game_events.IsEventByType(m_text.at(i).first));
    ...
    void GameEventSettings::SetChecked(utl::usize const index, bool const isChecked) {
        ...
        appContext.constants.h_game_events.SetEventByType(m_text.at(index).first, isChecked);

        assert(m_checkBoxes.size() == m_text.size());

        for (utl::usize i = 0; i < m_checkBoxes.size(); ++i) {
            m_checkBoxes.at(i)->SetChecked(appContext.constants.h_game_events.IsEventByType(m_text.at(i).first));
        }
    }

    GameEventSettings::GameEventSettings(utl::usize const focusID,
                                         Vector2 const pos,
                                         Vector2 const size,
    ...
    void GameEventSettings::SetRandom() {
        ...
        for (utl::usize i = 1; i < m_checkBoxes.size(); ++i) { // 0 is the global checkbox
            ...
            appContext.constants.h_game_events.SetEventByType(m_text.at(i).first, r);
            m_checkBoxes.at(i)->SetChecked(appContext.constants.h_game_events.IsEventByType(m_text.at(i).first));
        }
        m_checkBoxes.at(0)->SetChecked(appContext.constants.h_game_events.IsEventByType(utl::GameEventType::GLOBAL));
    }
} // namespace ui
