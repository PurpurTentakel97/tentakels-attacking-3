...
private:
    ...
    } }; ///< contains the llambda that gets called on save -> provides the current value

    bool m_isEnabled{ true }; ///< contains if the scene is enabled
    bool m_slided{
        false
    }; ///< contains if the scene is currently sliding -> locks the button if the value in the input line chanced because of the slider
